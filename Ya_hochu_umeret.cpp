// Method 1.cpp : Этот файл содержит функцию "main". Здесь начинается и заканчивается выполнение программы.
//
...
void print(std::string str, std::vector<std::vector<std::tuple<std::string, std::string, int,int>>>& start) {
...
std::pair<std::string,int> pash(std::vector<std::vector<std::tuple<std::string, std::string, int,int>>>&start_func, int k, int i) {
	...
	for (int j = 0; j < k; j++) {
		...
		if (flag) { vec.push_back({ std::get<0>(start_func[i][std::get<2>(start_func[i][j]) - 1]), "" , 0,k});
		flag = false;
		}
...
std::vector<std::vector<std::tuple<std::string,std::string, int,int>>> start_func(int size_vec, int size_str, std::vector<int> num, std::vector<std::vector<std::tuple<std::string, std::string, int,int>>> start) {
	bool flag = true;
	int k = 0, z = 0;
...
std::vector<std::vector<std::tuple<std::string, std::string, int,int>>> iniz(int size_str, int size_vec) {
...
void check_null(std::string str, std::vector<std::vector<std::tuple<std::string, std::string, int,int>>> &start) {
	print(str, start);
...
std::pair<std::string,int> manom(std::vector<std::tuple<std::string, std::string, int, int>> & start) {
	int k = 0;
	std::string str;
	for (int i = 0; i < start.size() - 1; ++i) {
		...
		else if (std::get<0>(start[k]) != "" && std::get<0>(start[i + 1]) == "") continue;
		else if (std::get<0>(start[k]) != "" && std::get<0>(start[i + 1]) != "") {
...
bool check_matrix(std::vector<std::vector<std::tuple<std::string, std::string, int, int>>> start) {
	bool flag = false;
	for (int i = 0; i < start.size(); ++i) {
		for (int j = 0; j < start[i].size(); ++j) {
			if (!start[i].empty()) {
...
void check_index(std::vector<std::vector<std::tuple<std::string, std::string, int,int>>>& start, int num) {
	std::string str;
	S2.clear();
	...
	for (auto i = 0; i < start.size(); ++i) {
		if (!start[i].empty())
		{
			...
			for (int j = 0; j < start[i].size(); ++j) {
				...
				if (std::get<0>(start[i][j]) != "" && check(std::get<0>(start[i][j]), t.first, std::get<3>(start[i][j]), t.second)) {
	...
	print(func, start);
		...
		for (auto i = 0; i < start.size(); ++i) {
			...
			if (!start[i].empty())
			{
				t = manom(start[i]);
				for (int j = 0; j < start[i].size(); ++j) {
					if (std::get<0>(start[i][j]) != "" && std::get<0>(start[i][j]) != t.first) {
						++k;
					}
				}
		...
		for (auto i = 0; i < start.size(); ++i) {
			...
			if (!start[i].empty())
			{
				...
				for (int j = 0; j < start[i].size(); ++j) {
					if (std::get<0>(start[i][j]) != "" && S2.find(std::get<0>(start[i][j])) != S2.end()) {
						start[i].clear();
						break;
					}
				}
...
void end_print(std::string str, std::vector<std::vector<std::tuple<std::string, std::string, int, int>>>& start) {
	std::cout << "F= ";
